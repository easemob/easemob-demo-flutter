...
2. 在 lib/main.dart 填写 App Server 的域名 `serverUrl` 和 `rtcAppId`。
...
│   └── demo_helper.dart       // demo 工具类
├── demo_config.dart           // demo 运行的配置类，包含 appKey、rtcAppId、serverUrl 等
...
├── pages
│   ├── call                                // 呼叫相关页面
│   │   ├── call_handler_widget.dart        // 呼叫监听页面，当 home 页启动时会初始化，用于监听语音呼叫回调。
...
│   │   └── group_member_select_view.dart   // 群成员选择页面，用于多人呼叫时选择参与人
│   ├── contact
...
├── tool
│   ├── app_server_helper.dart              // appServer 请求封装，用于封装向 AppServer 的请求
│   ├── format_time_tool.dart               // 时间工具，用于格式化通话时间
│   ├── online_status_helper.dart           // 在线状态工具类
│   ├── presence_status_helper.dart         // Presence 状态工具类
...
└── widgets
    ├── list_item.dart                      // 设置页面的 item
    ├── online_icon_status_widget.dart      // 在线状态图标组件
    ├── presence_icon_status_widget.dart    // Presence 状态图标组件
    ├── presence_title_widget.dart          // Presence 标题组件
    └── verify_code_widget.dart             // 验证码组件
```
